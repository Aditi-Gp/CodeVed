...
const successData = {
  ...
  difficulties: [["Easy", 200], ["Medium", 160], ["Hard", 52]],
};
...
const zeroData = {
  ...
  totalAttempted: 0,
  streakDays: 0,
  topics: [["Arrays", 0], ["DP", 0]],
  difficulties: [["Easy", 0], ["Medium", 0], ["Hard", 0]],
};

const feedItems = [
  ["Spatio-Temporal Anomaly Detection", "Accepted", "Python", "T-12m", true],
  ["Velocity Analysis Engine", "Wrong Answer", "Python", "T-3h", false],
  ["YOLO Architecture Core", "Accepted", "Python", "T-1d", true],
];
  ...
  useEffect(() => {
    ...
    const dataTimer = window.setTimeout(() => {
      ...
      loadingTimer = window.setTimeout(() => {
        if (scenario === "fetch_error") setView("error");
        else if (scenario === "empty") setView("empty");
        else {
          setData(scenario === "zero" ? zeroData : successData);
          setView("data");
        }
      }, 900);
    }, 600);
    ...
    <div className="relative z-[2] flex min-h-screen flex-col">
      <header className="sticky top-0 z-50 border-b-[3px] border-[var(--ink)] bg-[#f4f0e8]/95">
        <div className="mx-auto flex h-20 w-full max-w-[1404px] items-center justify-between px-8">
          ...
          <div className="border-2 border-[var(--ink)] bg-[var(--acid)] px-3 py-1.5 font-mono text-[13px] font-semibold shadow-[4px_4px_0_var(--ink)]">USER.DASHBOARD</div>
        </div>
      </header>

      <main className="mx-auto flex w-full max-w-[1404px] flex-1 flex-col px-8 pb-[120px] pt-[60px]">
        {view !== "data" ? <StateView view={view} /> : <DashboardData data={data} scenario={scenario} />}
      </main>

      <div className="fixed bottom-6 right-6 z-50 flex flex-wrap gap-3 border-2 border-[var(--acid)] bg-[var(--ink)] p-4 shadow-[8px_8px_0_var(--acid)] max-[700px]:bottom-3 max-[700px]:left-3 max-[700px]:right-3">
        {[["success", "Pass: OK"], ["zero", "Pass: 0/0"], ["auth_fail", "Fail: 401"], ["fetch_error", "Fail: Net"], ["empty", "Fail: Null"]].map(([value, label]) => (
          <button key={value} onClick={() => selectScenario(value)} className="border-2 border-[var(--ink)] bg-[var(--white)] px-3 py-1.5 font-mono text-[11px] font-semibold uppercase hover:bg-[var(--orange)] hover:text-[var(--white)]">{label}</button>
        ))}
      </div>
    </div>
  );
...
function StateView({ view }) {
  ...
  return (
    <section className={`relative mt-10 flex flex-1 flex-col items-center justify-center border-[3px] px-6 py-20 text-center shadow-[8px_8px_0_var(--ink)] ${isError ? "border-[var(--red)] bg-[#fff2ef] shadow-[12px_12px_0_var(--red)]" : "border-[var(--ink)] bg-[var(--white)]"}`}>
      ...
      {view === "authFailed" && <button onClick={() => { window.location.href = "/login"; }} className="mt-6 border-2 border-[var(--ink)] bg-[var(--white)] px-6 py-3 font-mono text-sm font-semibold uppercase shadow-[3px_3px_0_var(--ink)]">Return to Login</button>}
...
function DashboardData({ data, scenario }) {
      ...
      <div className="grid grid-cols-12 gap-8 max-[1100px]:flex max-[1100px]:flex-col">
        <BentoCard className="col-span-4 row-span-2 bg-[var(--acid)] shadow-[12px_12px_0_var(--ink)] hover:shadow-[16px_16px_0_var(--ink)]">
          ...
          <div className="flex flex-1 flex-col items-start justify-end">
            ...
            <div className="mb-3 font-serif text-[42px] font-medium leading-none tracking-[-.04em]">fluent_coder</div>
            <div className="mb-6 bg-[var(--ink)] px-3 py-1.5 font-mono text-[11px] font-semibold uppercase text-[var(--white)]">Knight Level</div>
            <div className="grid w-full grid-cols-2 gap-4 border-t-2 border-[var(--ink)] pt-4">{[["Rating", "1896"], ["Codeforces", "1046"], ["Primary", "Python"], ["Status", "Active"]].map(([label, value]) => <div key={label} className="flex flex-col"><span className="font-mono text-[10px] font-semibold uppercase text-[var(--muted)]">{label}</span><span className="text-xl font-bold">{value}</span></div>)}</div>
          </div>
        </BentoCard>
        ...
        <BentoCard className="col-span-8 bg-[var(--ink)] text-[var(--paper)]"><CardHeader title="Activity Matrix (T-30)" dark /><Heatmap /></BentoCard>
        ...
        <BentoCard className="col-span-12"><CardHeader title="Execution Log" action="view full output ↗" /><Feed empty={scenario === "zero"} /></BentoCard>
      </div>
    </div>
  );
}
...
function CardHeader({ title, action, dark = false }) { return <div className={`mb-6 flex items-center justify-between border-b-2 pb-4 font-mono text-xs font-semibold uppercase tracking-[.1em] ${dark ? "border-[#333]" : "border-[var(--ink)]"}`}><span><b className="mr-2 text-[var(--orange)]">///</b>{title}</span>{action && <a href="#" className="text-[var(--orange)]">{action}</a>}</div>; }
function Heatmap() { return <div className="flex min-h-[116px] flex-1 items-end gap-1.5 overflow-hidden">{Array.from({ length: 35 }, (_, column) => <div key={column} className="flex flex-col gap-1.5">{Array.from({ length: 3 }, (_, row) => { const level = (column * 7 + row * 3) % 6 > 2 ? ((column + row) % 4) + 1 : 0; return <div key={row} title={level ? `Vol: ${level * 12}` : "Idle"} className={`h-4 w-4 cursor-crosshair rounded-full transition hover:scale-125 hover:border hover:border-[var(--white)] ${level === 0 ? "bg-[#222]" : level === 1 ? "bg-[#4052d6] shadow-[0_0_4px_#4052d6]" : level === 2 ? "bg-[var(--orange)] shadow-[0_0_6px_var(--orange)]" : level === 3 ? "bg-[var(--acid)] shadow-[0_0_8px_var(--acid)]" : "bg-[var(--white)] shadow-[0_0_10px_var(--white)]"}`} />; })}</div>)}</div>; }
...
function Feed({ empty }) { if (empty) return <div className="border-2 border-[var(--ink)] p-10 text-center font-mono text-sm font-semibold uppercase text-[var(--muted)]">[ Buffer Empty ]</div>; return <div className="border-2 border-[var(--ink)]">{feedItems.map(([name, verdict, lang, time, accepted], index) => <div key={name} className={`flex items-center justify-between border-b-2 border-[var(--ink)] bg-[var(--white)] p-5 last:border-0 hover:bg-[var(--acid)] max-[700px]:items-start max-[700px]:gap-4 ${index === feedItems.length - 1 ? "" : ""}`}><div className="flex items-center gap-6 max-[700px]:items-start max-[700px]:gap-3"><span className={`border-2 border-[var(--ink)] px-3 py-1.5 font-mono text-[11px] font-bold uppercase text-[var(--white)] shadow-[2px_2px_0_var(--ink)] ${accepted ? "bg-[var(--green)]" : "bg-[var(--red)]"}`}>{verdict}</span><span className="text-lg font-bold max-[700px]:text-sm">{name}</span></div><div className="flex gap-6 font-mono text-sm font-semibold text-[var(--muted)] max-[700px]:flex-col max-[700px]:gap-1 max-[700px]:text-right"><span className="border-l-2 border-[var(--line)] pl-6 max-[700px]:border-0 max-[700px]:p-0">{lang}</span><span>{time}</span></div></div>)}</div>; }
